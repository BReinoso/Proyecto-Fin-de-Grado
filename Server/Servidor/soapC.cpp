...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.8.21 2015-04-03 16:54:40 GMT")
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_ns__sub(struct soap *soap, const char *tag, int id, const struct ns__sub *a, const char *type)
{
	...
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_ns__sub), type))
		return soap->error;
	if (soap_out_int(soap, "ns:a", -1, &a->a, ""))
		return soap->error;
	if (soap_out_int(soap, "ns:b", -1, &a->b, ""))
		return soap->error;
	return soap_element_end_out(soap, tag);
}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_a && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_int(soap, "ns:a", &a->a, "xsd:int"))
				{	soap_flag_a--;
					continue;
				}
			if (soap_flag_b && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_int(soap, "ns:b", &a->b, "xsd:int"))
				{	soap_flag_b--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_ns__subResponse(struct soap *soap, const char *tag, int id, const struct ns__subResponse *a, const char *type)
{
	...
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_ns__subResponse), type))
		return soap->error;
	if (a->result)
		soap_element_result(soap, "ns:result");
	if (soap_out_PointerToint(soap, "ns:result", -1, &a->result, ""))
		return soap->error;
	return soap_element_end_out(soap, tag);
}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_result && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_PointerToint(soap, "ns:result", &a->result, "xsd:int"))
				{	soap_flag_result--;
					continue;
				}
			soap_check_result(soap, "ns:result");
			if (soap->error == SOAP_TAG_MISMATCH)
				soap->error = soap_ignore_element(soap);
			if (soap->error == SOAP_NO_TAG)
				break;
